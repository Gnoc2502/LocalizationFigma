...
(() => __awaiter(void 0, void 0, void 0, function* () {
    const apiKey = (yield figma.clientStorage.getAsync('gemini_api_key')) || '';
    const model = (yield figma.clientStorage.getAsync('gemini_model')) || 'gemini-2.5-flash';
    const prompt = (yield figma.clientStorage.getAsync('gemini_prompt')) || '';
...
let _cachedSelectedNodeId = null;
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (msg.type === 'SAVE_SETTINGS') {
        yield figma.clientStorage.setAsync('gemini_api_key', msg.apiKey || '');
        yield figma.clientStorage.setAsync('gemini_model', msg.model || 'gemini-2.5-flash');
        yield figma.clientStorage.setAsync('gemini_prompt', msg.prompt || '');
        return;
    }
    if (msg.type === 'EXTRACT_TEXT') {
        const selectedNode = yield getSelectedFrame();
        if (!selectedNode) {
    ...
    if (msg.type === 'APPLY_TRANSLATION') {
        ...
        if (msg.sourceFrameId) {
            const node = yield figma.getNodeByIdAsync(msg.sourceFrameId);
            if (node && (node.type === 'FRAME' || node.type === 'SECTION' || node.type === 'COMPONENT' || node.type === 'COMPONENT_SET')) {
                originalFrame = node;
            }
        }
        else {
            originalFrame = yield getSelectedFrame();
        }
        if (!originalFrame) {
    ...
    if (msg.type === 'DETECT_OVERFLOW') {
        ...
        if (overflows.length > 0) {
            ...
            for (const o of overflows) {
                const overflowNode = yield figma.getNodeByIdAsync(o.nodeId);
                if (!overflowNode || !('absoluteBoundingBox' in overflowNode))
                    continue;
                const bounds = overflowNode.absoluteBoundingBox;
                if (!bounds)
                    continue;
                ...
                const parentFrame = yield figma.getNodeByIdAsync(frameId);
                if (parentFrame && 'appendChild' in parentFrame) {
                    parentFrame.appendChild(marker);
                    // Position relative to parent
                    marker.x = bounds.x - parentFrame.absoluteTransform[0][2] - 2;
                    marker.y = bounds.y - parentFrame.absoluteTransform[1][2] - 2;
                }
            }
        }
        figma.ui.postMessage({ type: 'OVERFLOW_RESULT', frameId, overflows });
    ...
    if (msg.type === 'GET_SOURCE_INFO') {
        const node = yield getSelectedFrame();
        if (!node) {
